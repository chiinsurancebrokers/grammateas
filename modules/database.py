...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS μέλη (
        ...
        κινητό              TEXT,
        email               TEXT,
        αρ_μητρώου_στοάς    TEXT,          -- αύξων αριθμός Στοάς
        αρ_μητρώου_μσ       TEXT,          -- αύξων αριθμός Μεγάλης Στοάς
        τεκτονικός_βαθμός   TEXT DEFAULT 'Μαθητής',
        ημ_μύησης           TEXT,
        ημ_εταίρου          TEXT,
        ημ_διδασκάλου       TEXT,
        στοά_μύησης         TEXT,          -- για υιοθετηθέντα/τακτοποιηθέντα
    ...
    CREATE TABLE IF NOT EXISTS συνεδριάσεις (
        ...
        ώρα                 TEXT,
        βαθμός              TEXT NOT NULL,          -- "Μαθητής"|"Εταίρος"|"Διδάσκαλος"
        τόπος               TEXT,
        πλήθος_παρόντων     INTEGER DEFAULT 0,
        αλληλογραφία        TEXT,                   -- JSON list
        ομιλίες             TEXT,                   -- JSON list
        αποφάσεις           TEXT,                   -- JSON list
    ...
    CREATE TABLE IF NOT EXISTS παρουσίες (
        ...
        παρών               INTEGER DEFAULT 1,       -- 1=παρών, 0=απών
    ...
    CREATE TABLE IF NOT EXISTS πρωτόκολλο (
        ...
        ημερομηνία          TEXT NOT NULL,
        κατεύθυνση          TEXT NOT NULL,   -- "Εισερχόμενο"|"Εξερχόμενο"
        αποστολέας          TEXT,
        παραλήπτης          TEXT,
        θέμα                TEXT NOT NULL,
        περιγραφή           TEXT,
    ...
    CREATE TABLE IF NOT EXISTS εντάλματα (
        ...
        ημερομηνία          TEXT NOT NULL,
        βιβλίο              TEXT NOT NULL,   -- "Γενικό"|"Ελεονομείο"
        ποσό                REAL NOT NULL,
        αιτιολογία          TEXT NOT NULL,
        δικαιολογητικά      TEXT,            -- JSON list
        αρ_απόφασης         TEXT,
        κατάσταση           TEXT DEFAULT 'Εκκρεμές',
        ημ_πληρωμής         TEXT,
        παρατηρήσεις        TEXT,
    ...
    CREATE TABLE IF NOT EXISTS μεταβολές (
        ...
        τύπος               TEXT NOT NULL,
        περιγραφή           TEXT,
        αναγγελία_μσ        TEXT,           -- ημ. αναγγελίας στη Μεγάλη Στοά
        αρ_πρωτ_μσ          TEXT,           -- ΑΠ Μεγάλης Στοάς
        παρατηρήσεις        TEXT,
        ημ_εγγραφής         TEXT DEFAULT CURRENT_TIMESTAMP
    )""")
    ...
    CREATE TABLE IF NOT EXISTS δελτία_διπλώματα (
        id                  INTEGER PRIMARY KEY AUTOINCREMENT,
        μέλος_id            INTEGER NOT NULL REFERENCES μέλη(id),
        τύπος               TEXT NOT NULL,   -- "Δελτίο Αναγνωρίσεως"|"Δίπλωμα Διδασκάλου"
        βαθμός              TEXT,
        ημ_αίτησης          TEXT,
        ημ_έκδοσης          TEXT,
        ημ_παράδοσης        TEXT,
    ...
    CREATE TABLE IF NOT EXISTS πρακτικά_συμβουλίου (
        ...
        ημερομηνία          TEXT NOT NULL,
        ώρα                 TEXT,
        παρόντες            TEXT,            -- JSON list
        θέματα              TEXT,            -- JSON list
        αποφάσεις           TEXT,            -- JSON list
        παρατηρήσεις        TEXT
    )""")
...
def save_member(data: Dict) -> int:
    """Insert or update. Returns member id."""
    conn = get_conn()
    mid = data.pop("id", None)
    if mid:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE μέλη SET {fields} WHERE id=?", (*data.values(), mid))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO μέλη ({cols}) VALUES ({phs})", list(data.values()))
        mid  = cur.lastrowid
    conn.commit()
    conn.close()
    return mid
...
def get_members_dropdown(active_only: bool = True) -> pd.DataFrame:
    """Returns id, fullname for selectboxes."""
    conn = get_conn()
...
def get_member_stats() -> Dict:
    ...
    for β in ΒΑΘΜΟΙ:
        stats[β] = c.execute("SELECT COUNT(*) FROM μέλη WHERE τεκτονικός_βαθμός=? AND κατάσταση='Ενεργός'", (β,)).fetchone()[0]
    conn.close()
    return stats
...
def get_session(sid: int) -> Optional[Dict]:
    ...
    if not row:
        conn.close()
        return None
    cols = [d[0] for d in conn.execute("SELECT * FROM συνεδριάσεις LIMIT 0").description]
    conn.close()
    d = dict(zip(cols, row))
...
def save_session(data: Dict) -> int:
    ...
    if sid:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE συνεδριάσεις SET {fields} WHERE id=?", (*data.values(), sid))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO συνεδριάσεις ({cols}) VALUES ({phs})", list(data.values()))
        sid  = cur.lastrowid
    conn.commit()
    conn.close()
    return sid
...
def save_attendance_bulk(sid: int, rows: List[Dict]):
    ...
    for r in rows:
        conn.execute("""
            INSERT INTO παρουσίες (συνεδρίαση_id, μέλος_id, παρών, δικαιολογήθηκε, αναπλήρωσε_θέση, παρατηρήσεις)
            VALUES (?,?,?,?,?,?)
    ...
    total_parwn = sum(1 for r in rows if r["παρών"])
    conn.execute("UPDATE συνεδριάσεις SET πλήθος_παρόντων=? WHERE id=?", (total_parwn, sid))
    conn.commit()
    conn.close()
...
def next_proto_number(year: int) -> str:
    conn = get_conn()
    cnt = conn.execute(
        f"SELECT COUNT(*) FROM πρωτόκολλο WHERE ημερομηνία LIKE '{year}%'"
    ).fetchone()[0]
    conn.close()
    return f"{cnt+1}/{year}"
...
def save_proto(data: Dict) -> int:
    ...
    if pid:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE πρωτόκολλο SET {fields} WHERE id=?", (*data.values(), pid))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO πρωτόκολλο ({cols}) VALUES ({phs})", list(data.values()))
        pid  = cur.lastrowid
    conn.commit()
    conn.close()
    return pid
...
def save_entalma(data: Dict) -> int:
    ...
    if eid:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE εντάλματα SET {fields} WHERE id=?", (*data.values(), eid))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO εντάλματα ({cols}) VALUES ({phs})", list(data.values()))
        eid  = cur.lastrowid
    conn.commit()
    conn.close()
    return eid
...
def save_metavoli(data: Dict, auto_update_member: bool = True) -> int:
    ...
    if did:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE μεταβολές SET {fields} WHERE id=?", (*data.values(), did))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO μεταβολές ({cols}) VALUES ({phs})", list(data.values()))
        did  = cur.lastrowid

    # Αυτόματη ενημέρωση κατάστασης μέλους
    if auto_update_member:
        status_map = {
            ...
            "Αποβίωση": "Αποβιώσας",
            "Ανύψωση σε Εταίρο": None,    # handled separately
            "Ανύψωση σε Διδάσκαλο": None,
        }
        τύπος = data.get("τύπος", "")
        new_status = status_map.get(τύπος)
        if new_status:
            conn.execute("UPDATE μέλη SET κατάσταση=? WHERE id=?", (new_status, data["μέλος_id"]))
...
def get_pending_announcements() -> pd.DataFrame:
    """Μεταβολές που δεν έχουν αναγγελθεί στη ΜΣ."""
    conn = get_conn()
    df = pd.read_sql_query("""
...
def save_diabevaiosi_axiomatikou(data: Dict) -> int:
    ...
    if did:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE διαβεβαιώσεις_αξιωματικών SET {fields} WHERE id=?", (*data.values(), did))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO διαβεβαιώσεις_αξιωματικών ({cols}) VALUES ({phs})", list(data.values()))
        did  = cur.lastrowid
...
def save_diabevaiosi_myoumenou(data: Dict) -> int:
    ...
    if did:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE διαβεβαιώσεις_μυουμένων SET {fields} WHERE id=?", (*data.values(), did))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO διαβεβαιώσεις_μυουμένων ({cols}) VALUES ({phs})", list(data.values()))
        did  = cur.lastrowid
    conn.commit(); conn.close()
    return did
...
def save_aporriphtheis(data: Dict) -> int:
    ...
    if aid:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE απορριφθέντες SET {fields} WHERE id=?", (*data.values(), aid))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO απορριφθέντες ({cols}) VALUES ({phs})", list(data.values()))
        aid  = cur.lastrowid
    conn.commit(); conn.close()
    return aid
...
def save_chrysi_vivlos_entry(data: Dict) -> int:
    ...
    if cid:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE χρυσή_βίβλος SET {fields} WHERE id=?", (*data.values(), cid))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO χρυσή_βίβλος ({cols}) VALUES ({phs})", list(data.values()))
        cid  = cur.lastrowid
    conn.commit(); conn.close()
    return cid
...
def save_deltio(data: Dict) -> int:
    ...
    if did:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE δελτία_διπλώματα SET {fields} WHERE id=?", (*data.values(), did))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO δελτία_διπλώματα ({cols}) VALUES ({phs})", list(data.values()))
        did  = cur.lastrowid
    conn.commit(); conn.close()
    return did
...
def get_symvoulio_minutes() -> pd.DataFrame:
    conn = get_conn()
    df = pd.read_sql_query("SELECT id, ημερομηνία, ώρα FROM πρακτικά_συμβουλίου ORDER BY ημερομηνία DESC", conn)
    conn.close()
    return df
...
def get_symvoulio_minute(sid: int) -> Optional[Dict]:
    ...
    if not row:
        conn.close()
        return None
    cols = [d[0] for d in conn.execute("SELECT * FROM πρακτικά_συμβουλίου LIMIT 0").description]
    d = dict(zip(cols, row))
...
def save_symvoulio_minute(data: Dict) -> int:
    ...
    if sid:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE πρακτικά_συμβουλίου SET {fields} WHERE id=?", (*data.values(), sid))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO πρακτικά_συμβουλίου ({cols}) VALUES ({phs})", list(data.values()))
        sid  = cur.lastrowid
    conn.commit(); conn.close()
    return sid
...
def save_ergasia(data: Dict) -> int:
    ...
    if eid:
        fields = ", ".join(f"{k}=?" for k in data)
        conn.execute(f"UPDATE εργασίες SET {fields} WHERE id=?", (*data.values(), eid))
    else:
        cols = ", ".join(data.keys())
        phs  = ", ".join("?" * len(data))
        cur  = conn.execute(f"INSERT INTO εργασίες ({cols}) VALUES ({phs})", list(data.values()))
        eid  = cur.lastrowid
    conn.commit(); conn.close()
    return eid
