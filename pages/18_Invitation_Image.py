...
- σύμβολο (δαφνινό στεφάνι) ανάμεσα στον Σεβάσμιο και τον Γραμματέα
...
def create_invitation(
    ...
    photo_h = 440
    raw_photo = photo_bytes or load_asset("acropolis.jpg")
    ...
    # ── Υπογραφές: τίτλοι + σύμβολο κέντρου + ονόματα ─────────
    sig_y  = y + 8
    sig_lx = INNER_X + 170
    sig_rx = PAGE_W - INNER_X - 170

    # Τίτλοι χωρίς γραμμές
    center_at(draw, sig_lx, sig_y, "Ο Σεβάσμιος",  f_sig, NAVY)
    center_at(draw, sig_rx, sig_y, "Ο Γραμματεύς", f_sig, NAVY)

    # Κεντρικό σύμβολο (δαφνινό στεφάνι / symbol_center)
    sym_w, sym_h = 160, 130
    sym_x = (PAGE_W - sym_w) // 2
    if not paste_asset(img, symbol_center_bytes, sym_x, sig_y - 10, sym_w, sym_h, opacity=0.95):
        draw_wreath_symbol(draw, PAGE_W // 2, sig_y + 40, 36, GOLD_DARK)
    draw = ImageDraw.Draw(img)

    # Ονόματα κεφαλαία χωρίς τόνους (.upper() αφαιρεί τόνους)
    name_y = sig_y + 52
    if master.strip():
