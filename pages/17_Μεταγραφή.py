...
# ══════════════════════════════════════════════════════════════
# CLAUDE → ΠΡΑΚΤΙΚΑ  (πλήρως εκπαιδευμένο)
# ══════════════════════════════════════════════════════════════
_SYSTEM_PROMPT = f"""
...
════════════════════════════════════════════════════════════════
ΥΠΟΔΕΙΓΜΑ ΣΤΥΛ — ΑΚΡΙΒΩΣ ΕΤΣΙ ΠΡΕΠΕΙ ΝΑ ΓΡΑΦΕΙΣ
════════════════════════════════════════════════════════════════

Κάθε ενότητα γράφεται σε ΠΛΗΡΕΙΣ ΠΑΡΑΓΡΑΦΟΥΣ, όχι τηλεγραφικά.
Κάθε παράγραφος: 3–6 προτάσεις. Γλώσσα: επίσημη αλλά σύγχρονη —
ΟΧΙ υπερβολικά αρχαϊκή. Χρησιμοποίησε ενεστώτα ή αόριστο οριστικής,
ΟΧΙ παρακείμενο αρχαίου τύπου παντού.

ΠΑΡΑΔΕΙΓΜΑ ΣΩΣΤΗΣ ΕΝΟΤΗΤΑΣ «ΟΜΙΛΙΑ»:
---
Ο Φιλτ∴ Αδ∴ Σεβ∴ παρέδωσε το Βήμα της Ευγλωττίας στον Ομιλητή της Εσπέρας,
ο οποίος ευχαρίστησε τον Σεβ∴ για την παραχώρηση και ιδιαίτερα τον επισκέπτη
Αδ∴ για την τιμητική παρουσία του.

Ο Αδ∴ Ομιλητής ανέπτυξε το θέμα [ΤΙΤΛΟΣ] αναφερόμενος πρώτα σε [ΚΥΡΙΑ ΕΝΝΟΙΑ 1].
Στη συνέχεια σύνδεσε [ΕΝΝΟΙΑ 1] με [ΕΝΝΟΙΑ 2], επισημαίνοντας ότι [ΒΑΣΙΚΟ ΣΥΜΠΕΡΑΣΜΑ].
Αναφέρθηκε στο Τυπικό και συγκεκριμένα στη φράση [ΦΡΑΣΗ ΤΥΠΙΚΟΥ], ερμηνεύοντάς την ως
[ΕΡΜΗΝΕΙΑ]. Κατέληξε ευχόμενος [ΕΥΧΗ].
---

ΠΑΡΑΔΕΙΓΜΑ ΣΩΣΤΗΣ ΕΝΟΤΗΤΑΣ «ΕΠΙ ΤΗΣ ΟΜΙΛΙΑΣ»:
---
Τον λόγο επί της ομιλίας έλαβαν κατά σειρά:

Αδ∴ [ΑΞΙΩΜΑ] [ΟΝΟΜΑ]: Επεσήμανε ότι [ΒΑΣΙΚΟ ΣΧΟΛΙΟ]. Αναφέρθηκε επίσης σε
[ΔΕΥΤΕΡΟ ΣΧΟΛΙΟ] και εξέφρασε [ΕΥΧΗ/ΑΠΟΤΙΜΗΣΗ].

Αδ∴ [ΑΞΙΩΜΑ] [ΟΝΟΜΑ]: [ΠΕΡΙΛΗΨΗ ΠΑΡΕΜΒΑΣΗΣ 3-4 ΠΡΟΤΑΣΕΩΝ].
---
...
ΚΡΙΣΙΜΟ για τους χαιρετισμούς (λίστα χαιρετισμοί_κατά_σειράν):
• Ο Μέγας Στεγαστής = «Ένδοξε Μέγ∴ Αξ∴» → ΤΑΞΗ 5 (πριν από πρώην Σεβ∴ που είναι τάξη 12)
• Πρώην Σεβάσμιοι = «Πρώην Σεβάσμιος» → ΤΑΞΗ 12
• ΠΑΝΤΑ ταξινόμησε χαιρετισμούς: Ένδοξε Μέγ∴ Αξ∴ πρώτος, μετά οι Πρώην Σεβάσμιοι
• Μορφή χαιρετισμού: "Ένδοξε Μέγ∴ Αξ∴ Δημήτριος Δενεδιός — Μέγας Στεγαστής Μ∴Σ∴Τ∴Ε∴"
• Μορφή χαιρετισμού: "Πρώην Σεβάσμιος Ελευθέριος Κεσσανής — πρώην Σεβάσμιος Σ∴ Στ∴"

ΚΡΙΣΙΜΟ — ΑΝΑΦΟΡΑ ΜΕΓΑ ΣΤΕΓΑΣΤΗ ΣΤΗΝ ΑΦΗΓΗΣΗ (κείμενο_πρακτικών):
• ΠΑΝΤΑ γράφε τον πλήρη τίτλο:
  "τον Ένδοξο Αδ∴ [ΟΝΟΜΑ ΕΠΩΝΥΜΟ], Μέγα Στεγαστή της Μεγάλης Στοάς της Ελλάδος"
  π.χ. "τον Ένδοξο Αδ∴ Δημήτριο Δενεδιό, Μέγα Στεγαστή της Μεγάλης Στοάς της Ελλάδος"
• ΟΧΙ "τον Ένδοξε Μέγ∴ Αξ∴" μέσα στο αφηγηματικό κείμενο.
• Δεύτερη αναφορά ίδιου εδαφίου: "ο Ένδοξος Αδ∴" ή "ο Μέγας Στεγαστής".

ΚΑΝΟΝΕΣ ΧΑΙΡΕΤΙΣΜΩΝ:
• Κατέγραψε ΟΛΟΥΣ όσους χαιρέτισε ο Σεβ∴ κατά την έναρξη
• ΜΗΝ βάζεις τον Σεβ∴ στους χαιρετισμούς — αυτός χαιρετά, δεν χαιρετιέται
• ΜΗΝ μπερδεύεις χαιρετισμούς με ομιλητές

ΚΑΝΟΝΕΣ ΟΜΙΛΗΤΩΝ:
• Κατέγραψε ΟΛΟΥΣ όσους έλαβαν τον λόγο, ΜΕ ΤΗ ΣΕΙΡΑ που μίλησαν
• Συμπεριέλαβε και τον Σεβ∴ αν μίλησε (έναρξη, κλείσιμο, σχόλια)
• Συμπεριέλαβε και τον νέο Γραμματέα αν μίλησε στα «υπέρ ευημερίας»
• Μορφή: "Σεβ∴ Διδ∴ Γεώργιος Παρίσης — καλωσόρισμα και έναρξη εργασιών"
...
Κάθε ενότητα ξεκινά με τίτλο σε κεφαλαία (π.χ. "ΕΝΑΡΞΙΣ — ΚΑΛΩΣΟΡΙΣΜΑ").
Χωρίζε ενότητες με κενή γραμμή.

ΥΠΟΧΡΕΩΤΙΚΕΣ ΕΝΟΤΗΤΕΣ (αν υπάρχουν στο κείμενο):

1. ΕΝΑΡΞΙΣ — ΚΑΛΩΣΟΡΙΣΜΑ
   Ο Φιλτ∴ Αδ∴ Σεβ∴ [ΟΝΟΜΑ] άνοιξε τας εργασίας της Σ∴ Στ∴ [ΟΝΟΜΑ ΣΤΟΑΣ] εις Βαθμ∴ [ΒΑΘΜΟΣ], καλωσορίζοντας
   θερμά όλους τους Αδδ∴, τους Αξιωματικούς και ιδιαίτερα [ΕΠΙΣΚΕΠΤΕΣ αν υπάρχουν],
   επισημαίνοντας [ΛΟΓΟ Ή ΧΑΡΑΚΤΗΡΙΣΜΟ ΤΗΣ ΕΣΠΕΡΑΣ αν υπάρχει].

2. ΕΠΙΚΥΡΩΣΙΣ ΠΡΟΗΓΟΥΜΕΝΩΝ ΠΡΑΚΤΙΚΩΝ
   Ανεγνώσθησαν τα Πρακτικά της Συνεδρίας της [ΗΜΕΡΟΜΗΝΙΑ], τα οποία ενεκρίθησαν
   παμψηφεί ως εχαράχθησαν.

3. ΑΛΛΗΛΟΓΡΑΦΙΑ / ΕΓΚΡΙΤΙΚΟΙ ΠΙΝΑΚΕΣ (αν υπάρχουν)
   Περίληψη εγγράφων ή αποφάσεων που αναγνώσθηκαν.

4. ΠΕΝΘΙΜΗ ΧΕΙΡΟΚΡΟΥΣΙΑ (αν υπάρχει)
   Ο Φιλτ∴ Αδ∴ Σεβ∴ κάλεσε το πλήρωμα όπως φέρη την πενθίμον χειροκρουσίαν
   εις μνήμην του [ΟΝΟΜΑ], [ΑΞΙΩΜΑ]. Το Εργαστήριο έφερε την πενθίμον
   χειροκρουσίαν τιμής ένεκεν.

5. ΕΓΚΑΤΑΣΤΑΣΗ ΑΞΙΩΜΑΤΙΚΩΝ (αν υπάρχει)
   Ο Φιλτ∴ Αδ∴ Σεβ∴ κάλεσε τους νέους Αξιωματικούς ενώπιον του Βωμού των
   Επίσημων Διαβεβαιώσεων, όπου έδωσαν τον καθορισμένο όρκο. Μετά την
   ολοκλήρωση, το Εργαστήριο απέδωσε τριπλή χαρμόσυνη χειροκρουσία.

6. ΟΜΙΛΙΑ ΕΣΠΕΡΑΣ (τίτλος + περίληψη 5-8 προτάσεων)
   Αναλυτική περίληψη του περιεχομένου της ομιλίας — όχι τηλεγραφική.
   Περιέλαβε τα κύρια επιχειρήματα, τις έννοιες του Τυπικού που αναλύθηκαν,
   τις αναφορές σε αρχαίους συγγραφείς/φιλοσόφους αν υπάρχουν, και το συμπέρασμα.

7. ΕΠΙ ΤΗΣ ΟΜΙΛΙΑΣ
   Κάθε παρέμβαση σε ξεχωριστή παράγραφο, 3-5 προτάσεων.
   Περίληψη των κύριων σημείων κάθε ομιλητή — ΟΧΙ "είπε θετικά πράγματα".

8. ΑΠΟΧΩΡΗΣΙΣ ΕΠΙΣΚΕΠΤΗ (αν υπάρχει)
   Πριν αποχωρήσει ο επισκέπτης, μετέφερε [ΤΙ ΕΙΠΕ ή ΤΙ ΜΕΤΕΦΕΡΕ].

9. ΕΠΙ ΤΗΣ ΕΥΗΜΕΡΙΑΣ (αν υπάρχει)
   Σύντομη περίληψη όσων ειπώθηκαν.

10. ΣΑΚΟΣ ΠΡΟΤΑΣΕΩΝ (αν υπάρχει)
    Ο Σάκος των Προτάσεων κόμισε [ΑΠΑΝΤΗΣΗ αν υπάρχει π.χ. δικαιολογία απόντος].

════════════════════════════════════════════════════════════════
ΤΙ ΝΑ ΜΗΝ ΣΥΜΠΕΡΙΛΑΒΕΙΣ στο "κείμενο_πρακτικών":
════════════════════════════════════════════════════════════════
• ΜΗΝ γράφεις ΚΛΕΙΣΙΜΟΝ / ΚΛΕΙΣΙΜΟ ενότητα — το κλείσιμο γίνεται αυτόματα
• ΜΗΝ γράφεις "Έλαβον τον λόγον:" λίστα — αυτή γίνεται αυτόματα
• ΜΗΝ γράφεις "Εν τω Τεκτ∴ Ναώ, τη..." — γίνεται αυτόματα
• ΜΗΝ γράφεις υπογραφές (Ο Σεβ∴ Διδ∴ / Ο Γραμματεύς / Ο Ρήτωρ)
• ΜΗΝ γράφεις "Μη υπάρχοντος ετέρου θέματος..." — γίνεται αυτόματα
• ΜΗΝ γράφεις "Είπον Σεβ∴ Διδ∴" — γίνεται αυτόματα
• ΜΗΝ γράφεις "Ο Κορμός εβλάστησεν..." — γίνεται αυτόματα από κορμός_αγαθοεργίας
Το "κείμενο_πρακτικών" τελειώνει ΠΑΝΤΑ με τον ΣΑΚΟ ΠΡΟΤΑΣΕΩΝ (ή με
ΕΠΙ ΤΗΣ ΕΥΗΜΕΡΙΑΣ αν δεν υπάρχει σάκος).

════════════════════════════════════════════════════════════════
ΓΛΩΣΣΙΚΟΙ ΚΑΝΟΝΕΣ
════════════════════════════════════════════════════════════════

ΣΩΣΤΟ ύφος (επίσημο, σύγχρονο):
• "Ο Φιλτ∴ Αδ∴ Σεβ∴ άνοιξε τας εργασίας της Σ∴ Στ∴ ΑΚΡΟΠΟΛΙΣ υπ' αρ. 84"
• "Ο Φιλτ∴ Αδ∴ Σεβ∴ κάλεσε το πλήρωμα να φέρη την πενθίμον χειροκρουσίαν"
• "Ο Αδ∴ Ομιλητής ανέπτυξε τη θεματική του καθήκοντος, επισημαίνοντας ότι..."
• "Εξέφρασε την ικανοποίησή του για την ατμόσφαιρα της συνεδρίασης"
• "Τόνισε ότι η εκπλήρωση καθήκοντος αποτελεί μέτρο εσωτερικής εξέλιξης"

ΛΑΘΟΣ (υπερβολικά αρχαϊκό — ΑΠΟΦΥΓΕ):
• "ανεγνώσθησαν και ενεκρίθησαν" → γράψε "αναγνώστηκαν και εγκρίθηκαν"
• "εκήρυξε την έναρξη" → "άνοιξε τας εργασίας της Σ∴ Στ∴"
• "άπαντες" → "όλοι" ή "άπαντες οι Αδδ∴" (εντάξει με τον Αδδ∴)

ΤΕΚΤΟΝΙΚΕΣ ΦΡΑΣΕΙΣ ΠΟΥ ΔΙΑΤΗΡΟΥΝΤΑΙ ΑΝΑΛΛΟΙΩΤΕΣ (μην τις αλλάζεις):
• "ως εχαράχθησαν" — επίσημη τεκτονική έκφραση για επικύρωση πρακτικών
• "εις Αιωνίαν Ανατολήν" — για μεταστάντα μέλη
• "τιμής ένεκεν" — για χειροκρουσία εις μνήμην
• "φέρωμεν την πενθίμον χειροκρουσίαν" — ΟΧΙ "αποδώσουμε" ή "αποδίδουμε"
• "μετά του οβολού του" — ΟΧΙ "με τον προβλεπόμενο οβολό του"
• Για την «Αντιγόνη» του Σοφοκλή: η σύγκρουση είναι "του απερχόμενου μινωικού
  με τον νέο μυκηναϊκό" — ΟΧΙ "μινωικού-μυκηναϊκού με αστικό-κανονιστικό"
• Στην ομιλία για το καθήκον — ΥΠΟΔΕΙΓΜΑ ΕΓΧΑΡΑΞΗΣ:
  Όταν η μεταγραφή περιλαμβάνει την ομιλία «Το καθήκον ως μέτρον της εσωτερικής
  εξελίξεως», το κείμενο των πρακτικών πρέπει να αποδίδει τα παρακάτω σημεία:

  Ο Αδ∴ Ομιλητής εισήγαγε ακόμη μια έννοια εκ του Τυπικού: «Ηθική είναι ο
  φυσικός και αναλλοίωτος νόμος που διέπει τα ελεύθερα όντα», ετυμολογούμενη
  εκ του "είθισται" — συνηθίζεται ως καθιερωμένος τρόπος διά της εμπειρίας
  που παράγει βέβαιον και ωφέλιμον αποτέλεσμα. Συνέδεσε τον ορισμό αυτόν με
  εκείνον του καθήκοντος: «η υποχρέωσις να φέρωμεν εις πέρας μια ευθύνη που
  αναλάβαμε», επισημαίνοντας ότι η εφαρμογή στην πράξη των τεκτονικών ιδεών
  αποτελεί καθήκον κατ' ελάχιστον ως προς τη σχέση με τους Αδδ∴.

  Υπενθύμισε τα τρία εμπόδια της πρώτης περιοδίας στον Α΄ Βαθμό και το ένα
  ενώπιον του Σεβ∴, τονίζοντας ότι όπως μεταξύ αγνοίας και γνώσης υπάρχει
  απόσταση, έτσι υπάρχει και μεταξύ γνώσης και πράξης. Και ακριβώς αυτό — το
  καθήκον της πράξεως — γίνεται το μέτρον της εσωτερικής μας εξελίξεως.

  Αναφέρθηκε στον Σοφοκλή — του οποίου το όνομα ετυμολογείται εκ του «κλέος
  της σοφίας» — και στο έργο του «Αντιγόνη», που πραγματεύεται, μεταξύ άλλων,
  την ηθική υπόσταση στη δομή της πολιτείας έναντι του νόμου. Ανέλυσε την
  ετυμολογία: Ετεοκλής = «δόξα γνήσιας καταγωγής», Πολυνείκης = «πολύ νεῖκος»
  (έριδα, έχθρα). Τέκνα του Οιδίποδα, ανήκουν στη «καταραμμένη γενιά» των
  Λαβδακιδών, που όπου βρεθούν φέρουν συμφορές εκ του μιάσματος που κουβαλούν.

  Το δίλημμα του έργου — αν θα αποδοθούν ταφικές τιμές στον Ετεοκλή ή θα
  παραμείνει άταφος ο Πολυνείκης — ανέλυσε ο Αδ∴ Ομιλητής ως σύγκρουση δύο
  διαφορετικών εποχών: του απερχόμενου μινωικού πολιτισμού (3600–1900 π.Χ.),
  με εξελιγμένη γλώσσα, τεράστια υποδομή και παγκόσμια ταυτότητα, με τον νέο
  μυκηναϊκό (1800–1100 π.Χ.), πιο αστικό και κανονιστικό. Παρατήρησε ότι η
  εμφάνιση νόμων προϋποθέτει συνήθως συμβάντα που απαγορεύει, και ότι ο
  Σοφοκλής θέτει δια του έργου το ερώτημα: γιατί μερικοί άνθρωποι διαχρονικώς
  δεν σέβονται τους θεσμούς που κληρονόμησαν και χρειάζονται επιπρόσθετη
  επιβολή νόμων;

  Κατέληξε ότι η Τεκτονική Αδελφότητα έχει ως βασική αποστολή να προσφέρει
  στον μυούμενο εφόδια για την εξέλιξή του σε πνεύμα ψυχικής ελευθερίας, καθότι
  η απελευθέρωση από την άγνοια και τα πάθη είναι το ζητούμενο των τεκτονικών
  εργασιών. Η πνευματική δυσκαμψία αποτελεί ενδογενές εμπόδιο — αυτό που
  συναντά ο μυούμενος στην Ανατολή — και δεν μπορεί να υπερβεί κανείς μόνος του
  χωρίς τη χείρα βοηθείας του Δοκιμαστή και του Τελετάρχη, ενώπιον του Σεβ∴
  με ό,τι αυτός συμβολίζει. Ευχήθηκε στους νέους Αξιωματικούς προθυμία και καλή
  προαίρεση στην εκπλήρωση των καθηκόντων τους, όπως ορίζει ο Γενικός
  Κανονισμός και ο Καταστατικός Χάρτης, υπό τη διεύθυνση του Φιλτ∴ Αδ∴ Σεβ∴.
• "Βήμα της Ευγλωττίας" — για τον άμβωνα ομιλίας
• "Βιβλίον των Παρουσιών" — για το βιβλίο παρουσιολογίου
• "Βωμός των Επισήμων Διαβεβαιώσεων" — για τον βωμό όρκου
• "άνοιξε τας εργασίας" — ΟΧΙ "εκήρυξε την έναρξη"

ΑΠΟΔΕΚΤΕΣ τεκτονικές εκφράσεις (κράτα τις):
• Φιλτ∴ Αδ∴ Σεβ∴ Διδ∴, Αδ∴, Αδδ∴, Σ∴ Στ∴
• Βήμα της Ευγλωττίας, Εργαστήριο, Στήλες της Ανατολής
• Αιωνία Ανατολή, Κορμός Αγαθοεργίας, Τεκτ∴ Ναός
• Σφύρα, πλήρωμα, χαρμόσυνη/πενθίμον χειροκρουσία
...
• Στο πεδίο "γραμματεύς" του JSON βάλε τον ΝΕΟ Γραμματέα.
• Στο πεδίο "ρήτωρ" βάλε τον ΝΕΟ Ρήτορα.
• Αν δεν προκύπτει ρητά ποιος είναι νέος, βάλε αυτόν που υπογράφει στο τέλος.
...
{{
  ...
  "χαιρετισμοί_κατά_σειράν": [
    "Ένδοξε Μέγ∴ Αξ∴ Όνομα Επώνυμο — Μέγας Στεγαστής Μ∴Σ∴Τ∴Ε∴",
    "Πρώην Σεβάσμιος Όνομα Επώνυμο — πρώην Σεβάσμιος Σ∴ Στ∴"
  ],
  "ομιλητές_κατά_σειράν": [
    "Σεβ∴ Διδ∴ Όνομα Επώνυμο — καλωσόρισμα και έναρξη εργασιών",
    "Αιδεσιμ∴ πρ∴ Σεβ∴ Όνομα Επώνυμο — κυρία ομιλία: τίτλος",
    "Αδ∴ Β΄ Επόπτης Όνομα — σχόλια επί της ομιλίας"
  ],
...
# Διορθώνει ∴, προσφωνήσεις και πτώσεις στο JSON output του Claude
# ══════════════════════════════════════════════════════════════

# Χαρτογράφηση: pattern (χωρίς ∴) → σωστή γραφή (με ∴)
_TRIA_REPLACEMENTS = [
    # Φιλτ∴ Αδ∴ Σεβ∴ (διάφορες γραφές)
...
# Αντικαταστάσεις προσφωνήσεων (πλήρεις → καθαρές)
_HONORIFIC_REPLACEMENTS = [
    # Αιδεσιμ* πρ* Σεβ* → Πρώην Σεβάσμιος
    (r"Αιδεσιμ[∴\.ώτα]*\.?\s*πρ[∴\.ώην]*\.?\s*Σεβ[∴\.]*",
     "Πρώην Σεβάσμιος"),
    # Ένδοξος Μέγ* Αξ* → Ένδοξε Μέγ∴ Αξ∴ (vocative στις προσφωνήσεις)
    (r"Ένδοξος\s+Μέγ[∴\.]*\s*Αξ[∴\.]*",
     "Ένδοξε Μέγ∴ Αξ∴"),
    # Ενδοξότατος → Ενδοξότατε (vocative)
...
def _normalize_tektonic_text(text: str) -> str:
    """
    Προσθέτει ∴ στις τεκτονικές συντομογραφίες και διορθώνει
    προσφωνήσεις/πτώσεις που ο Claude αποδίδει λανθασμένα.
    """
    if not text:
        return text
    import re as _re
    # Προσφωνήσεις πρώτα (πριν τα ∴, για να μην μπλέξουν τα patterns)
...
def _normalize_result_fields(result: dict) -> dict:
    """Εφαρμόζει κανονικοποίηση σε όλα τα text πεδία του JSON."""
    text_fields = [
...
def format_into_praktiko(
    ...
    try:
        ...
        # ── Post-process: κανονικοποίηση κειμένου (∴, προσφωνήσεις) ──
...
def generate_praktiko_pdf(d: dict) -> io.BytesIO:
    ...
    # ── Στυλ ─────────────────────────────────────────────────
    # ΣΗΜΑΝΤΙΚΟ: Χρήση DejaVuSans (fs/fsb) παντού — το DejaVuSerif
    # δεν έχει το σύμβολο ∴ (U+2234) και εμφανίζει □
    def S(name, **kw):
    ...
    # ══ ΟΜΙΛΗΤΕΣ — αφαιρέθηκε από εδώ, υπάρχει ήδη στην αφήγηση ═══
    ...
    if σεβ:
        closing_1 = (
            ...
            f"Σφύραν του Σεβ∴ Διδ∴ <b>{xe(σεβ)}</b>, των Αδδ∴ της Σ∴ Στ∴ ημών "
            f"ευχαριστημένων και ικανοποιημένων."
        )
    else:
        closing_1 = (
            "Μη υπάρχοντος ετέρου θέματος, αι Εργασίαι έκλεισαν κανονικώς, "
            "των Αδδ∴ της Σ∴ Στ∴ ημών ευχαριστημένων και ικανοποιημένων."
        )
    closing_2 = (
        "Οι Αδδ∴, αφού διεβεβαίωσαν να τηρήσουν σιγήν επί των Εργασιών, "
